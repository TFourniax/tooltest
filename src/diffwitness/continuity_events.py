...
def _event_integrity(event: dict[str, Any]) -> tuple[int, str, str]:
    ...
    has a leading comma. Removing that entire member preserves canonical JSON.
    Exactly one occurrence proves we removed the root member: it is always
    present, and any nested lookalike forces the independent standard fallback.
    Stored hashes never substitute for recomputation; raw input spelling is not
    used. Nonstandard Python inputs retain the original encoder behavior.
    """
    if (type(event) is not dict or any(type(key) is not str for key in event)
            or "actor" not in event
    ...
    if canonical.count(identity_member) != 1 or canonical.count(hash_member) != 1:
        return len(canonical) + 1, _event_id(event), _event_hash(event)
    hash_bytes = canonical.replace(hash_member, b"", 1)
    identity_bytes = hash_bytes.replace(identity_member, b"", 1)
    return (len(canonical) + 1, "dwev_" + hashlib.sha256(identity_bytes).hexdigest()[:24],
...
def validate_project_events(events: Iterable[dict[str, Any]]) -> _ProjectEventValidator:
        ...
        try:
            with path.open("rb") as handle:
                for number, raw in enumerate(handle, start=1):
                    digest.update(raw)
                    # Retain support for CR-only line endings and exact-byte anchors.
