...
def call_user_owned_provider(
    *,
    ...
    request = urllib.request.Request(_validate_url(endpoint), data=body, headers=headers, method="POST")
    try:
...
def _print_units(result: Mapping[str, Any]) -> None:
    ...
    print("\nAI provider: user-owned. DiffWitness managed inference cost: €0.")


def user_inference_cli(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(
        prog="dw explain",
        description="Explain the latest exact-bound change deterministically or with user-owned inference.",
    )
    ...
    try:
        ...
        print(f"IdleProof AI enhancement unavailable: {exc}", file=sys.stderr)
