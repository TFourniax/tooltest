...
def init_bare(path: Path) -> None:
    ...
    if proc.returncode:
        raise RuntimeError(proc.stderr)
    # Test remotes are removed immediately. `git receive-pack` can start detached automatic
    # maintenance after a push, which can race TemporaryDirectory cleanup on slower macOS runners.
    # Disable only this fixture housekeeping; transport semantics stay unchanged.
